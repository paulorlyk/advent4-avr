...
/*	causes occasional "move" with no describe & descitem	*/
	...
	if (dbugflg)
		printf_P(PSTR("loc = %d, verb = %d, object = %d, motion = %d\n"),
...
void dotrav(void)
{
	...
	for (kk = 0; travel[kk].tdest >= 0 && !mvflag; ++kk) {
		...
		if (dbugflg)
			printf_P(PSTR("rdest = %d, rverb = %d, rcond = %d, robject = %d in dotrav\n"),
	...
	if (!mvflag)
		...
		newloc = rdest;
		if (dbugflg)
			printf_P(PSTR("newloc in dotrav = %d\n"), newloc);
	}
}
...
void score(void)
{
	...
	printf_P(PSTR("Treasures:          %d\n"), s = t);
	t = (MAXDIE - numdie) * 10;
	if (t)
		printf_P(PSTR("Survival:           %d\n"), t);
	s += t;
	if (!gaveup)
		s += 4;
	t = dflag ? 25 : 0;
	if (t)
		printf_P(PSTR("Getting well in:    %d\n"), t);
	s += t;
	t = closing ? 25 : 0;
	if (t)
		printf_P(PSTR("Masters section:    %d\n"), t);
	s += t;
	if (closed) {
		if (bonus == 0)
			t = 10;
...
void trobj(void)
{
	if (verb)
		...
		printf_P(PSTR("What do you want to do with the %s?\n"), probj(object));
}
...
void dwarves(void)
{
	...
	*/
	...
	if (dtotal == 0)
		return;
	if (dtotal > 1)
		printf_P(PSTR("There are %d threatening little dwarves in the room with you!\n"), dtotal);
	else
		rspeak(4);
	if (attack == 0)
		return;
	if (dflag == 2)
		++dflag;
	if (attack > 1) {
		printf_P(PSTR("%d of them throw knives at you!!\n"), attack);
		k = 6;
	} else {
		rspeak(5);
		k = 52;
	...
	if (stick <= 1) {
		...
		printf_P(PSTR("%d of them get you !!!\n"), stick);
	oldloc2 = newloc;
	death();
}
/*
